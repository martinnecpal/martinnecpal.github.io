...
/**
 * PresentationsLoader class - handles loading and rendering presentations
 */
class PresentationsLoader {
    /**
     * @param {string} language - Language code ('en' or 'sk')
     * @param {string} jsonPath - Path to the presentations JSON file
     */
    constructor(language = 'sk', jsonPath = null) {
        this.language = language;
        this.jsonPath = jsonPath || `/${language}/presentations.json`;
        this.translations = TRANSLATIONS[language] || TRANSLATIONS.sk;
    }
    ...
    async loadPresentations() {
        try {
            ...
            if (presentations.length === 0) {
                container.innerHTML = `<p>${this.translations.noPresentation}</p>`;
                return;
            }
            ...
            presentations.filter(presentation => !presentation._help).forEach(presentation => {
                const presentationDiv = this.createPresentationElement(presentation);
            ...
            document.getElementById('presentations-container').innerHTML =
                `<p>${this.translations.loadError}</p>`;
        }
    }
    ...
    createPresentationElement(presentation) {
        ...
        const imageHtml = this.createImageHtml(presentation);
        const titleHtml = this.createTitleHtml(presentation);
        const buttonHtml = this.createButtonHtml(presentation);
        const notesHtml = this.createNotesHtml(presentation);
        const tagsHtml = this.createTagsHtml(presentation);

        presentationDiv.innerHTML = `
            ${imageHtml}
            <div class="presentation-content">
                ${titleHtml}
                <p class="presentation-meta"><strong>${this.translations.date}</strong> ${this.formatDateRange(presentation)}</p>
                <p class="presentation-meta"><strong>${this.translations.location}</strong> ${presentation.location}</p>
    ...
    createImageHtml(presentation) {
        if (presentation.photo) {
            ...
            return `<div class="presentation-placeholder">${this.translations.noImage}</div>`;
        }
    }
     ...
     */
    createNotesHtml(presentation) {
        if (presentation.notes) {
            return `<div class="presentation-notes">
                        <strong>${this.translations.notes}</strong> ${presentation.notes}
                    </div>`;
        }
        return '';
    }
     ...
     */
    formatDateRange(presentation) {
        const startDate = presentation.start_date || presentation.date;
        const endDate = presentation.end_date;

        if (!startDate) {
            return this.translations.noDate;
        }

        const formatOptions = { year: 'numeric', month: 'numeric', day: 'numeric' };
        const startFormatted = new Date(startDate).toLocaleDateString(this.translations.dateLocale, formatOptions);

        if (endDate && endDate !== startDate) {
            const endFormatted = new Date(endDate).toLocaleDateString(this.translations.dateLocale, formatOptions);
